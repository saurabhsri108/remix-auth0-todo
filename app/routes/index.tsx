...
export default function Index() {
  ...
  return <main className="bg-white text-gray-800 dark:bg-slate-800  dark:text-white flex flex-col justify-start gap-2 h-full sm:h-screen">
    <section className="flex flex-col justify-center items-center gap-4 w-full p-4">
      ...
      <Form ref={addingFormRef} method="post" className="flex flex-row gap-2 w-full md:w-1/2 justify-center items-center">
        <input ref={taskRef} className="w-2/4 md:w-3/4 p-3 border border-slate-400 dark:border-white focus:outline-slate-900 active:outline-slate-900 dark:text-gray-800 rounded-sm" type="text" name="task" id="task" placeholder='Eg. Buy vegetables...' />
        <button className="border border-slate-800 dark:border-white rounded-sm py-3 px-3 cursor-pointer w-auto md:w-1/4 bg-slate-800 text-white hover:bg-slate-900" type="submit" name='_action' value='create' disabled={isAdding}>Add to list</button>
      </Form>
    </section>
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-start items-start gap-2">
        {tasks.map((task: ITaskItem) => {
          return <li key={task.id} className={`flex flex-col gap-2 p-4 border border-slate-400 rounded-md min-h-full justify-between items-between ${task.completed && 'border-l-4 border-l-purple-500'}`}>
            ...
            <div className="font-sans font-medium flex flex-row gap-2 justify-between items-center mt-4">
              <span>{task.date}</span>
              <Form method='post'>
                <input type="hidden" name="id" value={task.id} />
                <button className="border border-purple-700 rounded-sm py-2 px-2 cursor-pointer w-auto bg-purple-700 text-white hover:bg-purple-800" type="submit" name='_action' value='done' disabled={isDoing}>{task.completed ? "Todo" : "Done"}</button>
              </Form>
            </div>
          </li>;
        })}
